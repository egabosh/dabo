...
function above {
  ...
  # above close[_5m] close[_1d] 2 [ECONOMY_SP500] [1h]

  ## Get and check values

  local f_name1=${1%%_*}  # if f_name1 is above f_name2 +
  local f_name2=${2%%_*}  # if f_name2 is below f_name1 -
  ...
  if [[ -z "$f_timeframe1" ]] || [[ -z "$f_timeframe2" ]]
  then
  ...
  if ! g_num_valid_number "$f_value1" "$f_value2" 2>/dev/null
  then
  ...
  ## check for above
  if g_num_is_higher $f_value1 $f_value2 && g_num_is_higher $f_value1_last $f_value2_last
  then
