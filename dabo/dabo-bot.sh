...
do
  ...
  # Get current list of supported exchanges by CCXT once a day
  if [ ! -f "$FILE" ] || [ $(( $(date +%s) - $(stat -c %Y "$FILE") )) -gt $MAXAGE ]
  then
    f_ccxt "print(ccxt.exchanges)"
    echo $f_ccxt_result | jq -r '.[]' >CCXT_SUPPORTED_EXCHANGES
  fi
